...
@section('main')
    <section class="section">
        <div class="section-header">
            <h1>Data Kategori Pengumuman</h1>
            ...
            <div class="card">
                ...
                <div class="card-body px-4">
                    <div class="table-responsive">
                        <table class="table-striped table-md table" id="kategoriTable">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Nama Kategori</th>
                            ...
                            {{-- <?php $no = ($kelas->currentPage() - 1) * $kelas->perPage() + 1; ?>
                            @foreach ($kelas as $k)
                                <tr>
                                    <td>{{ $no }}</td>
                                    <td>{{ $k->nama_kelas }}</td>
                                    ...
                                    <td class="row">
                                        <a href="{{ route('kelas.edit', $k->id) }}" class="btn btn-success btn-action mr-1"
                                            data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Tooltip on top" ><i class="fas fa-pencil-alt"></i></a>
                                        <form id="delete-form-{{ $k->id }}"
                                            action="{{ route('kelas.destroy', $k->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger"
                                                onclick="confirmDelete(event, 'delete-form-{{ $k->id }}')"><i
